...
with dai.Device(p.getOpenVINOVersion()) as device:
    ...
    for name in ["left", "right"]:
        ...
        # queues.append(device.getOutputQueue(name="rgb", maxSize=4, blocking=False))
# 
    ...
    while True:
        ...
        for i in range(2):
            ...
            if inConfig is not None:
                xmin = int(600 * inConfig.getCropXMin())
                ymin = int(600 * inConfig.getCropYMin())
                xmax = int(600 * inConfig.getCropXMax())
                ymax = int(600 * inConfig.getCropYMax())
